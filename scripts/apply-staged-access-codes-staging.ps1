...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260902_staged_access_code_rotation.sql"
)).Path
$testPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\tests\staged_access_code_rotation_staging.sql"
)).Path
...
Write-Host "Applying staged access-code migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" --single-transaction --variable ON_ERROR_STOP=1 `
  --file /migrations/20260902_staged_access_code_rotation.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging access-code migration failed and was rolled back."
}

Write-Host "Running rollback-only staged access-code tests"
& docker run --rm --mount $testMount $postgresImage psql `
  "--dbname=$databaseUrl" --variable ON_ERROR_STOP=1 `
  --file /tests/staged_access_code_rotation_staging.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging access-code tests failed."
}

Write-Host "Staged access-code migration and rollback-only tests passed."
