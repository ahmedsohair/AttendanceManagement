...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260903_invigilator_pagination.sql"
)).Path
$testPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\tests\invigilator_pagination_staging.sql"
)).Path
...
Write-Host "Applying invigilator pagination migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" --single-transaction --variable ON_ERROR_STOP=1 `
  --file /migrations/20260903_invigilator_pagination.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging invigilator pagination migration failed and was rolled back."
}

Write-Host "Running rollback-only invigilator pagination tests"
& docker run --rm --mount $testMount $postgresImage psql `
  "--dbname=$databaseUrl" --variable ON_ERROR_STOP=1 `
  --file /tests/invigilator_pagination_staging.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging invigilator pagination tests failed."
}

Write-Host "Invigilator pagination migration and rollback-only tests passed."
