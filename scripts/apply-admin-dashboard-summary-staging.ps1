...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260903_admin_dashboard_summary.sql"
)).Path
$testPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\tests\admin_dashboard_summary_staging.sql"
)).Path
...
Write-Host "Applying admin dashboard summary migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" --single-transaction --variable ON_ERROR_STOP=1 `
  --file /migrations/20260903_admin_dashboard_summary.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging admin dashboard summary migration failed and was rolled back."
}

Write-Host "Running rollback-only admin dashboard summary tests"
& docker run --rm --mount $testMount $postgresImage psql `
  "--dbname=$databaseUrl" --variable ON_ERROR_STOP=1 `
  --file /tests/admin_dashboard_summary_staging.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging admin dashboard summary tests failed."
}

Write-Host "Admin dashboard summary migration and rollback-only tests passed."
