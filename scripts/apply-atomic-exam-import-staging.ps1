...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260901_atomic_exam_import.sql"
)).Path
...
Write-Host "Applying atomic exam import migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" `
  --single-transaction `
  --variable ON_ERROR_STOP=1 `
  --file /migrations/20260901_atomic_exam_import.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging atomic exam import migration failed and was rolled back."
}
...
select
  exists (
    ...
    where oid = 'public.import_exam_session_atomic(uuid,text,date,text,jsonb,jsonb,text)'::regprocedure
  )
  and has_function_privilege(
    'service_role',
    'public.import_exam_session_atomic(uuid,text,date,text,jsonb,jsonb,text)',
    'EXECUTE'
  )
...
if ($LASTEXITCODE -ne 0 -or ($verifyResult | Select-Object -Last 1).Trim() -ne "t") {
  throw "Could not verify the staging atomic exam import function."
}

Write-Host "Staging atomic exam import migration completed and verified."
