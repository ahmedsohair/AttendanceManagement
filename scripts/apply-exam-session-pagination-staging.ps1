...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260903_exam_session_pagination.sql"
)).Path
$testPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\tests\exam_session_pagination_staging.sql"
)).Path
...
Write-Host "Applying exam session pagination migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" --single-transaction --variable ON_ERROR_STOP=1 `
  --file /migrations/20260903_exam_session_pagination.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging exam session pagination migration failed and was rolled back."
}

Write-Host "Running rollback-only exam session pagination tests"
& docker run --rm --mount $testMount $postgresImage psql `
  "--dbname=$databaseUrl" --variable ON_ERROR_STOP=1 `
  --file /tests/exam_session_pagination_staging.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging exam session pagination tests failed."
}

Write-Host "Exam session pagination migration and rollback-only tests passed."
