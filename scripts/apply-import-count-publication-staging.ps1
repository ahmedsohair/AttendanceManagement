...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260903_validate_import_counts_on_publish.sql"
)).Path
$testPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\tests\import_count_publication_staging.sql"
)).Path
...
Write-Host "Applying import-count publication validation to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" --single-transaction --variable ON_ERROR_STOP=1 `
  --file /migrations/20260903_validate_import_counts_on_publish.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging import-count publication migration failed and was rolled back."
}

Write-Host "Running rollback-only import-count publication tests"
& docker run --rm --mount $testMount $postgresImage psql `
  "--dbname=$databaseUrl" --variable ON_ERROR_STOP=1 `
  --file /tests/import_count_publication_staging.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging import-count publication tests failed."
}

Write-Host "Import-count publication migration and rollback-only tests passed."
