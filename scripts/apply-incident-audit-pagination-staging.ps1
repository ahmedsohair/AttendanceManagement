...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260903_incident_audit_pagination.sql"
)).Path
$testPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\tests\incident_audit_pagination_staging.sql"
)).Path
...
Write-Host "Applying incident audit pagination migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" --single-transaction --variable ON_ERROR_STOP=1 `
  --file /migrations/20260903_incident_audit_pagination.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging incident audit pagination migration failed and was rolled back."
}

Write-Host "Running rollback-only incident audit pagination tests"
& docker run --rm --mount $testMount $postgresImage psql `
  "--dbname=$databaseUrl" --variable ON_ERROR_STOP=1 `
  --file /tests/incident_audit_pagination_staging.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging incident audit pagination tests failed."
}

Write-Host "Incident audit pagination migration and rollback-only tests passed."
