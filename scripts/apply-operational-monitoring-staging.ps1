...
$migrationPath = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "..\supabase\migrations\20260904_operational_monitoring.sql")).Path
$testPath = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "..\supabase\tests\operational_monitoring_staging.sql")).Path
...
Write-Host "Applying operational monitoring migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql "--dbname=$databaseUrl" --single-transaction --variable ON_ERROR_STOP=1 --file /migrations/20260904_operational_monitoring.sql
if ($LASTEXITCODE -ne 0) { throw "Staging operational monitoring migration failed and was rolled back." }

Write-Host "Running rollback-only operational monitoring tests"
& docker run --rm --mount $testMount $postgresImage psql "--dbname=$databaseUrl" --variable ON_ERROR_STOP=1 --file /tests/operational_monitoring_staging.sql
if ($LASTEXITCODE -ne 0) { throw "Staging operational monitoring tests failed." }

Write-Host "Operational monitoring migration and rollback-only tests passed."
