...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260902_email_delivery_queue.sql"
)).Path
$testPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\tests\email_delivery_queue_staging.sql"
)).Path
...
Write-Host "Applying email delivery queue migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" --single-transaction --variable ON_ERROR_STOP=1 `
  --file /migrations/20260902_email_delivery_queue.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging email delivery queue migration failed and was rolled back."
}

Write-Host "Running rollback-only email delivery queue tests"
& docker run --rm --mount $testMount $postgresImage psql `
  "--dbname=$databaseUrl" --variable ON_ERROR_STOP=1 `
  --file /tests/email_delivery_queue_staging.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging email delivery queue tests failed."
}

Write-Host "Email delivery queue migration and rollback-only tests passed."
