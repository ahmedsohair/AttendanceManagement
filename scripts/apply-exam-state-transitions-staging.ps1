...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260902_exam_state_transitions.sql"
)).Path
$testPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\tests\exam_state_transitions_staging.sql"
)).Path
...
Write-Host "Applying exam-state transition migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" --single-transaction --variable ON_ERROR_STOP=1 `
  --file /migrations/20260902_exam_state_transitions.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging exam-state migration failed and was rolled back."
}

Write-Host "Running rollback-only exam-state tests"
& docker run --rm --mount $testMount $postgresImage psql `
  "--dbname=$databaseUrl" --variable ON_ERROR_STOP=1 `
  --file /tests/exam_state_transitions_staging.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging exam-state tests failed."
}

Write-Host "Exam-state migration and rollback-only tests passed."
