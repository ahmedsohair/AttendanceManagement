...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260901_atomic_attendance_mark.sql"
)).Path
...
Write-Host "Applying atomic attendance migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" `
  --single-transaction `
  --variable ON_ERROR_STOP=1 `
  --file /migrations/20260901_atomic_attendance_mark.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging atomic attendance migration failed and was rolled back."
}
...
select
  exists (
    select 1
    from pg_proc
    where oid = 'public.mark_attendance_atomic(uuid,uuid,text,uuid,text,text,text,boolean,text)'::regprocedure
  )
  and has_function_privilege(
    'service_role',
    'public.mark_attendance_atomic(uuid,uuid,text,uuid,text,text,text,boolean,text)',
    'EXECUTE'
  )
...
if ($LASTEXITCODE -ne 0 -or ($verifyResult | Select-Object -Last 1).Trim() -ne "t") {
  throw "Could not verify the staging atomic attendance function and privileges."
}

Write-Host "Staging atomic attendance migration completed and verified."
