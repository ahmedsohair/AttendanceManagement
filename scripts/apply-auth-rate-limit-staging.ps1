...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260901_auth_rate_limits.sql"
)).Path
...
Write-Host "Applying authentication rate-limit migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" `
  --single-transaction `
  --variable ON_ERROR_STOP=1 `
  --file /migrations/20260901_auth_rate_limits.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging rate-limit migration failed and was rolled back."
}
...
select
  ...
  and exists (
    select 1
    from pg_proc
    where oid = 'public.consume_auth_rate_limit(text,integer,integer,integer)'::regprocedure
  )
  and (
...
if ($LASTEXITCODE -ne 0 -or ($verifyResult | Select-Object -Last 1).Trim() -ne "t") {
  throw "Could not verify the staging rate-limit migration."
}

Write-Host "Staging authentication rate-limit migration completed and verified."
