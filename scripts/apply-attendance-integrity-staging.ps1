...
$migrationPath = (Resolve-Path -LiteralPath (
  Join-Path $PSScriptRoot "..\supabase\migrations\20260901_attendance_integrity_triggers.sql"
)).Path
...
Write-Host "Applying attendance integrity triggers to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" `
  --single-transaction `
  --variable ON_ERROR_STOP=1 `
  --file /migrations/20260901_attendance_integrity_triggers.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging attendance integrity migration failed and was rolled back."
}
...
select
  exists (
    ...
    where tgrelid = 'public.attendance_events'::regclass
      and tgname = 'attendance_events_validate_integrity'
      and not tgisinternal
  )
  and exists (
    select 1 from pg_trigger
    where tgrelid = 'public.incidents'::regclass
      and tgname = 'incidents_validate_integrity'
      and not tgisinternal
  );
"@
...
if ($LASTEXITCODE -ne 0 -or ($verifyResult | Select-Object -Last 1).Trim() -ne "t") {
  throw "Could not verify the staging attendance integrity triggers."
}

Write-Host "Staging attendance integrity triggers completed and verified."
