...
$migrationPath = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "..\supabase\migrations\20260903_confirm_exam_deletion.sql")).Path
$testPath = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "..\supabase\tests\confirm_exam_deletion_staging.sql")).Path
...
Write-Host "Applying typed exam deletion migration to staging project $targetProjectRef"
& docker run --rm --mount $migrationMount $postgresImage psql `
  "--dbname=$databaseUrl" --single-transaction --variable ON_ERROR_STOP=1 `
  --file /migrations/20260903_confirm_exam_deletion.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging typed exam deletion migration failed and was rolled back."
}

Write-Host "Running rollback-only typed exam deletion tests"
& docker run --rm --mount $testMount $postgresImage psql `
  "--dbname=$databaseUrl" --variable ON_ERROR_STOP=1 `
  --file /tests/confirm_exam_deletion_staging.sql
if ($LASTEXITCODE -ne 0) {
  throw "Staging typed exam deletion tests failed."
}

Write-Host "Typed exam deletion migration and rollback-only tests passed."
